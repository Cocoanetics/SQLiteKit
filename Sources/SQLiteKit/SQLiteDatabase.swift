...
public final class SQLiteDatabase {
    ...
    /// file is opened) under a recording authorizer. A sandboxing host can
    /// gate these through its own resolve + authorize path before executing
    /// the SQL for real — the same gate the database file passes — so `ATTACH`
    /// can't reach outside the sandbox.
    ///
